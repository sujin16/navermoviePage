...
<%@page import="kr.or.connect.navermovie.dto.Product"%>
<%@page import="java.util.List"%>
	...
	<div id="container">
		<div class="header">
			...
			<a href="bookinglogin.jsp" class="btn_my"> <span
				cl.conass="viewReservation" title="예약확인">예약확인</span>
			</a> </header>
		</div>
		<hr>
		<div class="event">
			...
			<div class="section_event_lst">
				...
				<div class="wrap_event_box"> 
                    	<c:forEach items="${list}" step="2" varStatus="i">
                    		<ul class="lst_event_box">
                    			<c:forEach var="j" begin="${i.index}" end="${i.index+2}">
								<c:choose>
									<c:when test="${i.index +2 eq j}">
										<c:set var="j" value="${i.index+3}"/>
									</c:when>
									<c:otherwise>
										<li class="item"><a href="detail.jsp" class="item_book">
												<div class="item_preview">
													<img alt="${list.get(j).getDescription()}" class="img_thumb"
												...
												<div class="event_txt">
													<h4 class="event_txt_tit">
														<span>${list.get(j).getDescription()}</span> <small class="sm">샤롯데 씨어터</small>
													</h4>
													<p class="event_txt_dsc">${list.get(j).getContent()}</p>
												</div>
										</a></li>
									</c:otherwise>
								</c:choose>
